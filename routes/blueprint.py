...
@bp.route("/twilio_call", methods=['POST'])
def twilio_call():
    try:
...
@bp.route("/twilio_message", methods=['POST'])
def twilio_message():
...
@bp.route('/interaction/<last_action>', methods=['GET', 'POST'])
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            if (interaction_type == "call"):
                ...
                return redirect(url_for('bp.call'), interaction_id=interaction.id)
            elif (interaction_type == "text"):
                # Send a text message
                logging.info("Redirecting to text message route...")
                return redirect(
                    url_for('bp.text_message', interaction_id=interaction.id))
            elif (interaction_type == "plan"):
                # Create an outreach agent and plan the outreach for the recipient
                logging.info("Redirecting to planning route...")
                return redirect(url_for('bp.plan', recipient_id=recipient.id))
...
@bp.route("/call/<interaction_id>", methods=['POST', 'GET'])
...
@bp.route("/text_message/<interaction_id>", methods=['POST', 'GET'])
...
@bp.route("/plan/<int:recipient_id>", methods=['GET', 'POST'])
